...
from .utils_image import random_sampling_in_polygons, binary_mask_to_polygon, polygon_areas, polygon_to_binary_mask_v2, ObjectProperties
...
def generate_nuclei_map(x, slide_size=None, n_classes=None, use_scores=False):
    ...
    val = x['scores'] if use_scores and 'scores' in x else [1.0] * len(i_c)
    # build point clouds
    pts = torch.sparse_coo_tensor([i_c.tolist(), i_x.tolist(), i_y.tolist()], 
                                  val, (n_classes, h, w)).coalesce()
...
def _regionprops(box, label, mask=None):
    w = max((box[2] - box[0] + TO_REMOVE), 1)
    h = max((box[3] - box[1] + TO_REMOVE), 1)
    o = {'box_area': h * w, 'labels': label}

    # mask = cv2.resize(mask[0].float().numpy(), (w, h), interpolation=cv2.INTER_LINEAR) > 0.5
    mask_w, mask_h = int(math.ceil(w)), int(math.ceil(h))
    if mask:
        mask = polygon_to_binary_mask_v2(mask, size=(mask_h, mask_w), mode='xy')
        if mask.sum():
            prop = ObjectProperties(np.array(box), mask)
...
def extract_nuclei_features(object_iterator, num_workers=None, **kwargs):
    ...
    if object_iterator.masks is None:
        ...
        return {'box_area': h * w, 'labels': labels}
    else:
