...
# @app.post("/proxy/dummy_files", status_code=201)
# async def backgound_jobs(request: Request, client=Depends(get_redis)):
#     request_args = {k.split('amp;')[-1]: v for k, v in request.query_params.items()}
#     image_id, registry = request_args['image_id'], request_args['registry']
#     key = f"{image_id}_{registry}"
...
# try:
    ...
    for tile_id in tile_ids:
        ...
        serialized_item = pickle.dumps({'image_id': image_id, 'registry': registry, 
...
async def test_connection():
    ...
    print(f"Test connection to redis ({redis}). Ping successful: {await client.ping()}")
    await client.aclose()
